...
TEMPLATES = {
    "7.0": ROOT / "templates" / "7.0" / "template_dovecot.yaml",
    "8.0": ROOT / "templates" / "8.0" / "template_dovecot.yaml",
}
...
def validate(path: Path, export_version: str) -> None:
    ...
    assert template.get("template") == "Dovecot by Zabbix agent", f"{path}: template technical name is not standardized"
    assert vendor.get("name") == "NetTech", f"{path}: vendor name is not standardized"
