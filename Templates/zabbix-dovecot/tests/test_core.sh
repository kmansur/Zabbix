...
RESULT="$(
    ...
    /bin/sh "$PROJECT_DIR/scripts/zabbix-dovecot" stats
)"
...
# 5. Version modes.
VERSION="$(DOVECOT_BINARY="$FAKE_DOVECOT" /bin/sh "$PROJECT_DIR/scripts/zabbix-dovecot" version)"
[ "$VERSION" = "2.4.5" ] || {
    printf 'FAIL: Dovecot version. Actual: %s\n' "$VERSION" >&2
    exit 1
}

COLLECTOR_VERSION="$(/bin/sh "$PROJECT_DIR/scripts/zabbix-dovecot" collector-version)"
